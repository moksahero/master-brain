...
MKT="${MB_MARKETPLACE_DIR:-$HOME/.claude/local-marketplaces/master-brain}"
...
if [ "$bump" != "none" ]; then
  ...
  run "perl -0pi -e 's/\"version\": \"${cur//./\\.}\"/\"version\": \"${new}\"/g' \
...
[ -d "$MKT" ] || { echo "ship: marketplace dir not found: $MKT" >&2; exit 1; }
